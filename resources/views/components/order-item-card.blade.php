@props([
    ...
    'name' => '',
    'price' => 0,
    'image' => 'public/images/dummy-item.png',
    'originalPrice' => null,
    'description' => null,
    'expanded' => false,
    'discount' => 0,
...
<div
    ...
    <div class="flex items-center gap-3 min-w-0">
        ...
        <div class="min-w-0 ml-1">
            <p
                class="text-subtitle font-bold text-gray-800 truncate leading-tight group-hover:text-primary transition-colors">
                {{ $name }}
            </p>
            @if ($description)
                <p class="text-2xs text-gray-400 font-medium truncate mt-0.5">{{ $description }}</p>
            @endif

            @if ($expanded)
                <span class="inline-block mt-1 text-2xs font-bold text-green-600 bg-green-50 px-2 py-0.5 rounded-md">
                    In Stock
                </span>
            @endif
        </div>
    ...
    <div class="flex items-center gap-3 shrink-0 ml-2">
        <div class="text-right">
            <p class="text-subtitle font-extrabold text-gray-900">${{ number_format($price, 2) }}</p>
            @if ($originalPrice)
                <p class="text-2xs text-gray-400 line-through decoration-red-400/50">
    ...
    @if ($expanded)
        <div class="grid grid-cols-2 gap-3 mt-4 pt-4 border-t border-gray-50">
            <div class="space-y-1">
                <label class="text-2xs uppercase font-bold text-gray-400 tracking-wider ml-1">Quantity</label>
            ...
            <div class="space-y-1">
                <label class="text-2xs uppercase font-bold text-gray-400 tracking-wider ml-1">Disc (%)</label>
                <div class="relative">
